...
import { setscrollervalue } from '../redux/Filter';
const categorys = [
    "Frontend Developer",
    "Backend developer",
    "Seo expert",
...
export default function Scroller() {
    ...
    useEffect(() => {
        const dipatched = dispatch(setscrollervalue(filter))
    }, [filter])
    return (
        <>
                            ...
                            categorys.map((category, index) => {
                                return (
                                    <CarouselItem key={category} className=" text-xl md:text-3xl basis-1/2 md:basis-1/3 ">
                                        <Button onClick={() => setfilter(category)} className='text-[#E1D9BC] bg-rounded-sm px-3 py-2 rounded-full  bg-[#30364F]'>{category}</Button>
                                    </CarouselItem>
                                )
                            })
                        }
